...
except Exception as _cv2_exc:  # pas seulement ImportError : _ARRAY_API etc. sont des AttributeError
    CV2_AVAILABLE = False
...
_DETECTOR_ERRORS: dict[str, str] = {}  # {"face": "...", "eye": "...", "hog": "..."}
    ...
    if _FACE_CASCADE is None and CV2_AVAILABLE:
        ...
        if bundled_path.exists():
            ...
            try:
                ...
                _DETECTOR_ERRORS["face"] = f"cv2.data indisponible : {exc}"
                return None
        try:
            loaded = cv2.CascadeClassifier(cascade_path)
            if loaded.empty():
                _DETECTOR_ERRORS["face"] = f"cascade vide chargée depuis {cascade_path}"
                _FACE_CASCADE = None
            else:
                _FACE_CASCADE = loaded
        except Exception as exc:
            ...
            _DETECTOR_ERRORS["face"] = f"{type(exc).__name__}: {exc}"
            _FACE_CASCADE = None
    ...
    if _EYE_CASCADE is None and CV2_AVAILABLE:
        ...
        if bundled_path.exists():
            ...
            try:
                ...
                _DETECTOR_ERRORS["eye"] = f"cv2.data indisponible : {exc}"
                return None
        try:
            loaded = cv2.CascadeClassifier(cascade_path)
            if loaded.empty():
                _DETECTOR_ERRORS["eye"] = f"cascade vide chargée depuis {cascade_path}"
                _EYE_CASCADE = None
            else:
                _EYE_CASCADE = loaded
        except Exception as exc:
            _DETECTOR_ERRORS["eye"] = f"{type(exc).__name__}: {exc}"
            _EYE_CASCADE = None
    return _EYE_CASCADE
...
def _get_hog_detector():
    ...
    if _HOG_PERSON_DETECTOR is None and CV2_AVAILABLE:
        try:
            ...
            _DETECTOR_ERRORS["hog"] = f"{type(exc).__name__}: {exc}"
            _HOG_PERSON_DETECTOR = None
    ...
    if SHOW_EDITION:
        if CV2_AVAILABLE:
            with ph_block(
                ...
                _cv2_version = getattr(cv2, "__version__", "inconnue") if CV2_AVAILABLE else "non importé"
                _np_version = getattr(np, "__version__", "inconnue") if CV2_AVAILABLE else "non importé"
                st.caption(f"🔎 OpenCV {_cv2_version} · NumPy {_np_version}")

                if not CV2_AVAILABLE:
                    st.error(f"⚠️ OpenCV n'a pas pu être importé du tout : {CV2_IMPORT_ERROR}")
                elif _face_ok and _eye_ok and _hog_ok:
                    ...
                    st.error(
                        f"⚠️ Détection **{', '.join(missing)}** indisponible. Les "
                        "photos concernées retomberont sur un cadre centré sur "
                        "l'image ENTIÈRE, pas sur le sujet."
                    )
                    with st.expander("Détail technique de l'erreur (pour diagnostic)"):
                        for key, label in [("face", "visage"), ("eye", "yeux"), ("hog", "silhouette")]:
                            if key in _DETECTOR_ERRORS:
                                st.code(f"{label} : {_DETECTOR_ERRORS[key]}")
