...
def frame_portrait_ruled(bgr):
    ...
    Repli (aucun visage détecté) : cadre carré centré sur l'image entière.
    """
    h, w = bgr.shape[:2]
    try:
        cascade = _get_face_cascade()
        if cascade is None:
            raise ValueError("no face cascade")
        gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
        faces = cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(60, 60))
        if len(faces) == 0:
        ...
        return _frame_on_white_canvas(bgr, frame_left, frame_top, frame_w, frame_h)
    except Exception:
        side = min(w, h)
        left, top = (w - side) / 2, (h - side) / 2
        return _frame_on_white_canvas(bgr, left, top, side, side)
